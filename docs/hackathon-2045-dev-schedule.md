...
**建议主赛道**：**PayFi**（托管 + 支付意图 + Mock HSP + Webhook）；**可选佐证**：并行 **x402**（见 [payfi-escrow-architecture.md](./payfi-escrow-architecture.md)）。**链上默认**：**Base Sepolia**。
...
- [ ] **主赛道**：PayFi（若强绑定 HashKey Chain，在 README 说明当前 **Base Sepolia 演示** + **适配层可迁链**，避免评审误解）。
